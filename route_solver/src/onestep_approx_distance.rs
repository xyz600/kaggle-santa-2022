...
#[derive(Clone)]
pub struct OneStepApproximateDistanceFunction {
    cell_list: Vec<Cell>,
}

impl OneStepApproximateDistanceFunction {
    pub fn load(filepath: &PathBuf) -> OneStepApproximateDistanceFunction {
        ...
        OneStepApproximateDistanceFunction { cell_list }
    }
}

impl DistanceFunction for OneStepApproximateDistanceFunction {
    fn distance(&self, id1: u32, id2: u32) -> i64 {
        ...
        let diff = dx + dy;
        let iter = (diff / 8) as f64;
        let rest = (diff % 8) as f64;

        let dist = 8.0f64.sqrt() * iter + rest.sqrt();
